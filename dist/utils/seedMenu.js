...
async function seedMenu() {
    ...
    await prisma_1.default.menuItem.deleteMany({});
    ...
    for (const item of menuItems) {
        await prisma_1.default.menuItem.upsert({
            where: { id: (await prisma_1.default.menuItem.findFirst({ where: { name: item.name, branchId: branch.id } }))?.id || 0 },
            update: item,
...
seedMenu().finally(() => prisma_1.default.$disconnect());
